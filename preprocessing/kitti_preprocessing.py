...
def combine_preprocess_for_train(image, labels, bboxes,
                         out_shape, data_format='NHWC',
                         scope='combine_preprocessing_train'):
    """Preprocesses the given image for training.
    ...
    """
    fast_mode = False
    with tf.name_scope(scope, 'combine_preprocessing_train', [image, labels, bboxes]):
        if image.get_shape().ndims != 3:
            raise ValueError('Input must be of size [height, width, C>0]')
        # Convert to float scaled [0, 1].
        if image.dtype != tf.float32:
...
def preprocess_image(image,
    ...
    """
    if is_training:
        return combine_preprocess_for_train(image, labels, bboxes,
                                    out_shape=out_shape,
