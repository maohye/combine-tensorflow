# 模型验证
"""Generic evaluation script that evaluates a combine model on a given dataset."""
import math
import time
...
tf.app.flags.DEFINE_string(
    'checkpoint_path', 'D:/graduate/code/combine/code/model.ckpt-109176',
...
tf.app.flags.DEFINE_string(
    'dataset_dir', 'D:/graduate/code/combine/code/data/tfrecords/VOC2007/TF_test', 'The directory where the dataset files are stored.')
tf.app.flags.DEFINE_string(
    'model_name', 'combine', 'The name of the architecture to evaluate.')
tf.app.flags.DEFINE_string(
...
def save_image_with_bbox(image, labels_, scores_, bboxes_):
    if not hasattr(save_image_with_bbox, "counter"):
        save_image_with_bbox.counter = 0  
    ...
    imsave(os.path.join('D:/graduate/code/combine/code/Debug', '{}.jpg').format(save_image_with_bbox.counter), img_to_draw)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device('/cpu:0'):
            with tf.name_scope(FLAGS.dataset_name + '_data_provider'):
                provider = slim.dataset_data_provider.DatasetDataProvider(
                    ...
                    shuffle=False)
            # Get for combine network: image, labels, bboxes.
            ...
            gclasses, glocalisations, gscores, _ = \
                combine_net.bboxes_encode(glabels, gbboxes, combine_anchors)
            batch_shape = [1] * 5 + [len(combine_anchors)] * 3
        ...
        # =================================================================== #
        # combine Network + Ouputs decoding.
        # =================================================================== #
        dict_metrics = {}
        ...
        with slim.arg_scope(arg_scope):
            predictions, logits, objness_pred, objness_logits, localisations, end_points = \
                combine_net.net(b_image, is_training=False)
        # Add loss function.
        ...
        with tf.device('/device:CPU:0'):
            localisations = combine_net.bboxes_decode(localisations, combine_anchors)
            ...
            rscores, rbboxes = \
                combine_net.detected_bboxes(filtered_predictions, localisations,
